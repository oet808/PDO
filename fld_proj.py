...
# index for a time-varying field
# and using eofs as vectors for the projection
# This script works with the PCA results
# of the North Pacific domain.
###############################################################################
...
for scen in SCENARIOLIST:
    ...
    for v in VARLIST:
        ...
        infile_eof=MODEL+"_"+cesmscen+"_"+v+"_"+cesmtime+"_"+run+'_ann_ano_pca_eof.nc'
        if RESID:
        ...
        print("field data: "+OUPATH+infile)
        print("eigenvectors from "+OUTPATH+infile_eof)
        print("output file: "+OUPATH+outfile)
        print ("call function to read the netcdf files")
        ### open the data sets ###
        nc1=xarray.open_dataset(SOURCE)
        ntime1=nc1.time.size
        field=nc1[v].values[:]
           
        nc2=xarray.open_dataset(SOURCE_EOF)
        eof=(nc2['eof'].values[:])
        nmodes=len(eof[:,0,0])
        #################################################
        ...
        # to the residuals
        #################################################
        is_lon1=np.logical_and(nc1.lon.values>=110,nc1.lon.values<=260)
        nlon1=np.sum(is_lon1)
        is_lat1=np.logical_and(nc1.lat.values>=20,nc1.lat.values<=70)
        ...
        # sizes
        is_lon2=np.logical_and(nc2.lon.values>=110,nc2.lon.values<=260)
        nlon2=np.sum(is_lon2)
        is_lat2=np.logical_and(nc2.lat.values>=20,nc2.lat.values<=70)
        ...
        while t<ntime:
            ...
            #print(t)
            t=t+1
        ds=save_result(proj,nc1.time,nc2.lev,copy_from_source=nc1[VAR]) 
        if False:
        ...
        print ("results written to "+OUTPATH+outfile)
    nmodel+=1
print ("done")
