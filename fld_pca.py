    ...
    for run in ENSEMBLELIST:
        for v in VARLIST:
            ...
            if RESID:
                ...
                outfile_eof=MODEL+"_"+cesmscen+"_"+v+"_"+cesmtime+"_"+run+"_ann_ano_resid_eof.nc"
                outfile_pc=MODEL+"_"+cesmscen+"_"+v+"_"+cesmtime+"_"+run+"_ann_ano_resid_pc.nc"
            else:
                infile=MODEL+"_"+cesmscen+"_"+v+"_"+cesmtime+"_"+run+"_ann_ano.nc" 
                outfile_eof=MODEL+"_"+cesmscen+"_"+v+"_"+cesmtime+"_"+run+"_ann_ano_eof.nc"
                outfile_pc=MODEL+"_"+cesmscen+"_"+v+"_"+cesmtime+"_"+run+"_ann_ano_pc.nc"
            ### open the data sets ###
...
#fig=plt.figure()
#plt.contourf(nc1.lon.values,nc1.lat.values,a,cmap=plt.cm.coolwarm)
#plt.colorbar()
#fig.show()

# compare time series at a point
#sel_lon=180
#sel_lat=0
#ilon=np.where(np.abs(nc1.lon-sel_lon)<1.25)[0][0]
#ilat=np.where(np.abs(nc1.lat-sel_lat)<1.25)[0][0]

#plt.plot(nc1.time.values,fielddata[:,ilat,ilon],color='green')
#plt.plot(nc2.time.values,x,color='black')
#plt.plot(nc1.time.values,res[:,ilat,ilon],color="cyan")
#plt.show()
